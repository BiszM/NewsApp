...
public class NewsModel extends Model {
    ...
    public static void deleteBookmarkedNews(){
        new Delete().from(NewsModel.class).where("bookmarked = ?", true).execute();
    }

    public static void deleteSavedNews(){
        new Delete().from(NewsModel.class).where("save = ?", true).execute();
    }
